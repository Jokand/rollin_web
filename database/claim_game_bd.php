...
if (isset($_POST)) {
    ...
    if (count($_SESSION['errors']) == 0) {
        $SQL = "INSERT INTO `games`( `title`, `record`, `link_to_illustration`, `description`, `remark`, `setting`, `system`, `genre`, `id_master`, `price`, `environment`, `location`, `dategame`, `beginning_game`, `end_game`) VALUES
        ('{$title}','{$record}','{$fileName}','{$description}','{$remark}','{$setting}','{$system}','{$genre}','{$id_master}','{$price}','{$environment}','{$location}','{$begin_date}','{$begin_time}','{$end_time}')";
        $database->query($SQL);
